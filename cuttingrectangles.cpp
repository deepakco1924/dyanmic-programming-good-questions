...
int find_ans(int a,int b){
  ...
  //either you cut from vericatl of horizontal
  //let curfrom the horizonal ;
  int ans=INF;
  for(int  i=1;i<a;i++){
    int op1=find_ans(i,b);
    int op2=find_ans(a-i,b);
    ans=min(ans,op1+op2+1LL);
  }
  for(int i=1;i<b;i++){
    int op1=find_ans(a,i);
    int op2=find_ans(a,b-i);
    ans=min(ans,op1+op2+1LL);
  }
  return dp[a][b]=ans;
}
